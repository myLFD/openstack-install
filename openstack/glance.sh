...
openstack-config --set /etc/glance/glance-api.conf DEFAULT transport_url rabbit://openstack:$rabbitmq_pass@controller:5672
openstack-config --set /etc/glance/glance-api.conf database connection mysql+pymysql://glance:$glance_pass@controller:3306/glance
...
mysql -uglance -p$glance_pass -e "use glance;show tables;"
...
#glance image-create --name "loongnix-server-password-changable" --file loongnix-server-new.qcow2 --disk-format qcow2 --container-format bare --visibility=public  --property hw_qemu_guest_agent=yes
