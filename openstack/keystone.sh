...
openstack-config --set /etc/keystone/keystone.conf database connection mysql+pymysql://keystone:$keystone_pass@controller:3306/keystone
...
#4.同步完成进行连接测试
mysql  -ukeystone -p$keystone_pass -e "use keystone;show tables;"
mysql -hcontroller -ukeystone -p$keystone_pass -e "use keystone;show tables;"|wc -l
